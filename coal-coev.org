...
*** Moran Susceptible-Infected-Susceptible (Moran + SIS)
Model: [[./slim/TwoSpecies_Moran_SIS.slim]] 

This model is simuilar to the Moran SI model, except, that each generation individuals can recover at rate $\gamma$. In this case, the pathogen they contain is killed and the host returns to being healthy. 

NOTE: This implementation of Moran Infection models has all pathogen potential infecting each generation, although there is still generation overlap (as not all infect sucessfully), the timescale at which the pathogens reproduce is much faster than that at which the host reproduces (which is often biologically realistic as well). If we wanted a true Moran of hosts and pathogens it could be desirable to only let one random pathogen infect each generation. This would make the model very slow though, especially when studying coalescence. To speed up the dynamics and study the coalescence of both hosts and pathogens, we implement the above mentioned model without generation overlap as a Cannings-type model.
