...
The implementation of Moran Infection models has a set of pathogens infect in each generation with probability $c$. This leads to a pathogen model with generation overlap, the patogens killed can be from any generation. While the host follows a Moran model of one individual per generation ($d = 1/N$), the timescale at which the pathogens reproduce is much faster than that at which the host reproduces (which is often biologically realistic as well). If we wanted a true Moran of hosts and pathogens it could be desirable to only let one random pathogen infect each generation. This would make the model very slow though, especially when studying coalescence (and the biological meaning is also not clear). To speed up the dynamics and study the coalescence of both hosts and pathogens, we implement the above mentioned model without generation overlap as a Cannings-type model. 

*** Wright-Fisher Susceptible-Infected (WF + SI)
Because generation do not overlap in the Wright-Fisher model, there needs to be some kind of vertical transmission mechansism, otherwise the pathogen will become extint within one generation.

Discrete generations Cannings model with Wright-Fisher host and pathogen reproduction. Host and pathogen offspring ($H_{t+1}$ and $P_{t+1}$) are sampled randomly with replacement from the current generation t. Individuals from t are then killed (non-overlapping generations).

This causes the coalescence timescales to be the simiar, compared to the Moran-based host pathogen models described above.
*Hypothesis*: Timescale ratio between Hosts and pathgoens will depend on transmission rate $c$ and hoost and pahogen population sizes.
...
- TODO create python model and slim structures to save and analyze co-treeSequences.
*Hypothesis*
