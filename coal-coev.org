...
#+caption: Scales of host-pathogen interactions. A) Epidemic dynamics of disease transmission and spread within a field. Adapted from [[cite:&luz-2010-model-trans]], [[cite:&kuehnert-2011-phylog-epidem]]. B) Metapopulation dynamics between fields. C) Coevolutionary interactions between hosts and pathogens drive the dynamics of their allele frequencies. Adapted from [[cite:&tellier-2021-theor-host]], [[cite:&zivkovic-2019-neutr-genom]], [[cite:&vienne-2013-cospec-vs]] and [[cite:&woolhouse-2002-biolog-biomed]]. D) Long-term coevolution between host and pathogen can lead to accosiations in their species phylogenies. Adapted from [[cite:&ebert-2020-host]] and [[cite:&dismukes-2022-cophy-method]]. Additional icons have been taken from [[https://biorender.com]]. 
...
** Two-species Infection models in SLiM 5
*** Moran Susceptible-Infected (Moran + SI)
Model: [[./slim/TwoSpecies_Moran_SI.slim]]
...
*What kind of model for the pathogen does this result in? Density-dependent growth?*

A variation of this exists as a Susceptible-Infected-Susceptible model, where a host can overcome the infection at a give rate $\gamma$. In this case, the pathogen they contain is killed and the host returns to being healthy. 
Model: [[./slim/TwoSpecies_Moran_SIS.slim]] 

The implementation of Moran Infection models has a set of pathogens infect in each generation with probability $c$. This leads to a pathogen model with generation overlap, the pathogens killed can be from any generation. While the host follows a Moran model of one individual per generation ($d = 1/N$), the timescale at which the pathogens reproduce is much faster than that at which the host reproduces (which is often biologically realistic as well). If we wanted a true Moran of hosts and pathogens it could be desirable to only let one random pathogen infect each generation. This would make the model very slow though, especially when studying coalescence (and the biological meaning is also not clear). To speed up the dynamics and study the coalescence of both hosts and pathogens, we implement the above mentioned model without generation overlap as a Cannings-type model. This causes the coalescence timescales to be the simiar, compared to the Moran-based host pathogen models described above.

*** Wright-Fisher Susceptible-Infected (WF + SI)
Because generation do not overlap in the Wright-Fisher model, there needs to be some kind of vertical transmission mechansism, otherwise the pathogen will become extint within one generation. WIth vertical transmission here we mean the transmission between discrete host generations, with pathogens either infecting random hosts or infecting the offspring of  their infected parents (classical vertical transmission). For sake of simplicity we model random vertical transmission for now, but hope to extend the model to include vertical transmission along lines of descent.

Discrete generations Cannings model with Wright-Fisher host and pathogen reproduction. Each generation host produce offspring sampled randomly with replacement, which will make up the hosts in the new generation $t+1$, leading to a Wright-Fisher multipnomial reproduction mechanism. After host offspring are generated the pathogens each reproduce to a random unique (drawn without replacement) host offspring. This causes a reshuffling of the infected hosts each generation, due to each pathogen choosing a new host to infect. In addition to vertical transmission of pathogens between discrete host generations, each pathogen individual also can reproduce horizontally to a random host. If this host is not infected previously by vertical or horizontal tranmsiion events, then the horizontal transmitting pathogen will reproduce to the choosen host.   

- [ ] add vertical transmission along lineages? Tuneable somehow?
- [X] add fitness scaling for infected hosts. Host offspring sampling weighted by 1-s.
- [ ] Host resistance loci and pathogen virulence loci with alleles (i.e. multiple host and pathogen types). Mutation or standing variation? 
- [ ] Infection matrix for infection outcomes between types
- [ ] Recombination between host and parasite types?
  
*** Generalized model
We aim to develop a model that generalizes some of the behaviours and limiting cases above. such as scaling between continuous and discrete parts, and pathogen-host interdependency (eg. lineage-constrained transmission vs free transmission)...

** Notes on considerations for the two Species model
...
** Coalescence timescale of host-pathogen co-genealogies
...
#+attr_latex: :width 1\linewidth  :center t :placement
[[./figures/coupled_genealogies.png]]
*** TODO Timescale in neutral host-pathogen models
...
- [X] create python model and slim structures to save and analyze co-treeSequences.
- [ ] Run discrete WF host pathogen model for different parameters of $c$ and $s$. 
- run c=0.01 for really slow transmission. 
- [ ] pairwise plots of the coalescence times
...
* BarleyCOPA meeting, 2025: Research overview :noexport:
** ARG-based GWAS
[[cite:&rebollo-2025-lever]]

1. Controlling for genetic relatedness in Genome-wide association studies (GWAS)

Branch based-relationship matrix (BRM) - [[cite:&lehmann-2025-args]], [[cite:&fan-2022-geneal-estim]]
* 1st TAC meeting, January 2026 :noexport:
_Underlying goals_:
1. To connecting genealogical structures to (co-)evolutionary and epidemiological processes, by considering the reproductive processes of the host and pathogens and the infection process that connects them.
2. To develop a framework of coupled host-pathogen genealogies from paired samples.
3. To use ARGs of hosts and pathogens to improve existing joint genomic inference (eg. genome-to-genome, co-GWAS)

_Questions about coevolutionary trees_:
- How do properties of coevolutionary dynamics affect joint genome evolution in host and pathogens from an tree-based perspective? (eg. infection matrix, horizontal vs. vertical transmission, cost of infection/resistance, co-infection, generation times).
- Can we recover / infer above parameters of the coevolutionary system from the knowledge of the joint ARGs, considering biases to ARG inference by model violations?

_Questions about pathogen ARGs_:
- Can we use reconstructed pathogen ARGs to conduct phylodynamic inference and gain insights in to pathogen transmission properties. Relevant for barley-crop pahogens: Characterizing dispersal and population structures between fields (epidemic within field probably not resolved enough). Also the question of recombination between and within strains/species of Fusarium.

_Questions about GxG inference_:
- Can host and pathogen ARGs be used to represent the evolutionary histories of our samples and imporve the inference power of genome-to-genome methods. (Similar to how ARGs can improve GWAS ([[cite:&rebollo-2025-lever]]))?
  
* Thesis Proposal :noexport:
...
- Epidemiology alone misses selection for host and pathogen types and their genomic evolution including mutation and recombination.
- Evolutionary models - without epidemiological dynamics - do not account for demographic structure and population dynamics, arising from a disease transmission (that can also be spatially structured). 
What is the special role of reproduction, infection and recombination in both epidemiology and evolution?

*** Genealogies and ancestral recombiantion graphs (ARGs) as despcriptors (?) of evolutionary history
