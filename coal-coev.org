...
- *$c_{v} = 1$, $X_{i} = 1$; $c_{h} = 1$:* discrete WF (pathogens find new random host each generation. Difference: pathogen population growth depends on host reproduction)
...
- *$n > 1$:* Polycyclic model. $n$ pathogen reproduction cycles per host reproduction. Seasonality?
