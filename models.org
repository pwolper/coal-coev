...
#+LATEX: \vspace{10pt}
#+ATTR_LATEX: :options [enhanced, title={Discrete time population genetics models of coevolutions}]
#+begin_tcolorbox
Population genetic models of coevolution are used to describe the change in frequencies of host and parasite alleles under selection.  

\begin{equation}
W_{H,i} = \sum_{j=1}^{A_P} P_j (1 - \alpha_{i,j} + (1-s_H)\alpha_{i,j}).
\end{equation}
#+end_tcolorbox
** TODO Semi-discrete
...
* Host and parasite genealogical branching models
