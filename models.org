...
* Existing coevolutionary models
...
Generally coevolutionary models differ by their inclusion of population dynamics (i.e. population size changes), which is known as eco-evolutionary feedback ([[cite:&ashby-2019-under-role]]). 
Constant population size models can be constructed with Moran-type birth death processes ([[cite:&macpherson-2020-coevol-does]] and [[cite:&macpherson-2021-feedb-between]]) or the Moran Infection process ([[cite:&welch-2005-integ-geneal-epidem]]), while existing ecological coevolutionary model builds upon ecological models such as Lotka-Volterra dynamics (eg.  [[cite:&frank-1993-specif-versus]]; [[cite:&nuismer-2017-introd]], Chapter 4; and [[cite:&macpherson-2021-feedb-between]], see Ecological Model) and includes density-dependent growth regulation. Coevolutionary models of epidemiology can be considered a form of ecological model, that is based on epidemiological compartmental model, such as the SI models (see Box below).

#+LATEX: \vspace{10pt}
#+ATTR_LATEX: :options [enhanced, title={Continuous time coevolutionary model based on epidemiological models}]
#+begin_tcolorbox
...
Here $\alpha_{ij}$ represents the infection matrix determining the infection outcome and A being the number of host and pathogen alleles, respectively. Costs of infection, resistance and a reduction in fecundity when infected are omitted here for simplicity (see [[cite:&boots-2014-how-specif]], [[cite:&zivkovic-2019-neutr-genom]]). 
#+end_tcolorbox
#+LATEX: \vspace{10pt}

Because the epidmiological differntial equations describe the dynamics of host compatments (i.e. susceptible and infected hosts), they do not model the pathogen population explicitly; usually the number of infected individuals serves as a proxy for the number of parasite individuals with the number $P_{j}$ of parasite of genotype $j$ being $P_{j} = \sum_{i=1}^{A_H} I_{ij}$, assuming one parasite per infected genotype. The total population size of the parasite is then given by $N_{P} = \sum_{j=1}^{A_j} P_{j}$. 

** TODO Discrete time models
Evolutionary models rely on discrete models, that assume constant population size. Allele frequency dynamics is only by the frequencies of host and pathogen types (density independent?)
[[cite:&tellier-2007-polym-multil]], [[cite:&tellier-2007-stabil-genet]]. Poly-cycllic or monocyclic diseases? 
  
#+LATEX: \vspace{10pt}
#+ATTR_LATEX: :options [enhanced, title={Discrete time population genetics models of coevolution}]
#+begin_tcolorbox
Population genetic models of coevolution describe the change in frequencies of host and parasite alleles under selection ([[cite:&leonard-1977-selec-press]], [[cite:&tellier-2007-stabil-genet]], Nuismer book). 
Assume the frequency of host allele $i$ in the current generation is defined as $H_{i}$. Following the Wright-Fisher model with selection the frequency of $i$ in the next generation depends on the host genotype fitness $W_{H,i}$ and the population mean fitness $W_{H}$:

\begin{equation}
H'_{i} = \frac{H_{i}W_{H,i}}{W_H}
\end{equation}

The fitness of hosts with genotype $i$ depends on the interaction with the pathogen, as infection will reduce the fitness of the host by $s_H$. Assuming each host encounters a random pathogen with any allele $j$ each generation, the host genotype fitness also depends on the summed product of the frequency of pathogen gentypes $j$ and their ability to infect host $i$, which is determined by the infection matrix $\alpha_{i,j}$.  

\begin{equation}
W_{H,i} = 1- s_H \sum_{j=1}^{A_P} P_{j} \alpha_{i,j}
\end{equation}

The same logic can be applied to the pathogen, with the fitness of the pathogen genotype $j$ depending on the hosts $i$ it is able to infect successfully. (Why only $H_{i}\alpha_{i,j}$?)

\begin{equation}
W_{P,j} = \sum_{i=1}^{A_H} H_{i} \alpha_{i,j}
\end{equation}

Putting all together we end up with recurrence expressions for the evolution of host and pathogen allele frequencies $H_{i}$ and $P_{j}$, respectively.

\begin{equation}
H'_{i} = \left[ H_{i} (1 - S_H(\sum_{j=1}^{A_P} P_{j} \alpha_{i,j})) \right]/W_H
\end{equation}

\begin{equation}
P'_{j} = \left[ P_{j} \sum_{i=1}^{A_H}\alpha_{i,j} \right] / W_P
\end{equation}
#+end_tcolorbox
** TODO Semi-discrete
...
* Host and parasite ancestral branching models
...
SIR-based coalescents and Phylodynamic models
** TODO Ancestral infection and selection graph, [[cite:&welch-2005-integ-geneal-epidem]]
Approach to formulate a coalescent for host and pathogens for a Moran Infection process [[cite:&welch-2005-integ-geneal-epidem]].
** TODO Co-phylogenetics for tree associations

* Genealogy-explicit co-evolutionary models
...
** TODO Moran Susceptible-Infected (Moran + SI)
...
** TODO Wright-Fisher Susceptible-Infected (WF + SI)
...
** A host-pathogen nested-Canning's models
To set up a generalized genealogy-specific host pathogen model we propose a nested Canning's model including infection and horizontal/vertical transmission. This can be described as separate Canning's reproduction processes for host and pathogen. To account for timescale differences such as differences in speed of evolution and reproduction the reproductive process of the pathogen can happen $n \times$ more rapidly than the host reproductive process (see Fig. [[generalized_vertical_model]]).

The model is in two phases: host reproduction and pathogen reproduction. Host reproduction proceeds by a Canning's-type reproduction, with each host $i \in N_{host}$ producing $Y_{i}$ offspring, with this number following an offspring distribution $\mathcal{D}_{Host}$. Each host $i$ can be infected by a pathogen $j$; one for now. During the host reproduction the parasite individuals follow their hosts with a probability $c_v$ of vertical transmission. As we consider a reproducing pathogen inside the host, the pathogen can duplicate if $Y_{i} < 1$. If a host dies, the pathogen that infected it, dies as well.
Host reproduction is followed by pathogen reproduction, with  the pathogen $j$ in each host producing $X_j$ propagules or offspring. This offspring can either stay within the host (allowing one pathogen per host or selective filtering) or can each migrate to another random host with probability $c_{h}$ of horizontal transmission 

#+LATEX: \vspace{10pt}

# TODO add tcolorbox to defined functions to change colors.
#+begin_tcolorbox :options 
\large *_Host-pathogen nested Canning's model_*
...
#+attr_latex: :width 1\linewidth  :center t :float nil
...
* Host pathogen epidemiological model with horizontal and vertical transmission (with Johannes Müller)
** TODO put introduction
** TODO insert drawing and mathematics
...
** [[cite:&zivkovic-2019-neutr-genom]] Model :noexport:
