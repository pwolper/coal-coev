...
Coevolutionary models include a wider-range of population and quantitative genetics models and approaches, that describe the coevolving dynamics of hosts and pathogens, occuring at evolutionary time scales. Most commonly these models are designed to include host and pathogen genotypes, infection genetics, different types of frequency-dependent selection and have readily been extended with mutation and recombination (*citation Nuismer book*). As such coevolutionary models have been used to describe rich dynamics; including cyclical dynamics and maintainance of polymorphisms through trench warefare; and unstable equilibria leading to fixations through arms race dynamics, to name a few.

Generally we identified two types of coevolutionary models: discrete-time population genetics models ([[cite:&tellier-2007-stabil-genet]], [[cite:&tellier-2014-speed-adapt]]) and continuous-time eco-evolutionary models ([[cite:&boots-2014-how-specif]], [[cite:&zivkovic-2019-neutr-genom]]). The discrete-time population genetics models, typically describe the change in allele frequencies of host and pathogens as recurrence equations, while continuous-time models are often based on differential equations and compartmental epidemiological models describing changes in host and pathogen population sizes. 

The importance of including population dynamics in coevolutionary models is recognized, as eco-evolutionary (epidemiological) feedbacks have a major impact on the model dynamics ([[cite:&buckingham-2022-coevol-theor]], [[cite:&macpherson-2021-feedb-between]]). Yet still the literature of coevolutionary models is split by the inclusion or exclusion of population dynamics, and including population dynamics increases the complexity of the models significantly. Recently, forward-time simulation frameworks capable of tracking ancestral genealogies under recombination and complex population dynamics (e.g. SLiM 5.0) have become possible, enabling the study of ancestry patterns generated by realistic coevolutionary models.

The genomic signatures arising in coevolutionary models are typically characterized with population-level summary statistics (eg. allele frequencies, diversity measures or linkage disequilibrium) (see [[cite:&tellier-2014-speed-adapt]], [[cite:&ebert-2020-host]]). While these approaches have yielded important insights into signatures of coevolutionary dynamics, the genealogical histories underlying recombining host and pathogen genomes under biologically realistic models of coevolution and epidemiology are rarely modelled explicitly.  
...
*TODO* Framework for joint trees from co-phylogeny and co-speciation fields (see [[cite:&dismukes-2022-cophy-method]], [[cite:&vienne-2013-cospec-vs]]). *cite Tangled trees chapter*

The work of [[cite:&welch-2005-integ-geneal-epidem]] gives a pathogen genealogy embedded in a host epidemioligcal model as the ancestral infection and selection graph.
* Research Gap
...
- How do inferred host and pathogen ARGs capture known genomic signatures of coevolutions and epidemiological dynamics?
- Can ARG-derived statistics be used to infer GxG interactions and infection matrices from data?   
- Can we extend tree-based phylodynamic inference to full ARGs?
- Guiding joint host-pathogen demographic inferences? (co-demography, see [[cite:&zivkovic-2019-neutr-genom]]?)
- How can we apply coevolutionary models and inference to data beyond using simple summary statistics? (eg. beyond pairwise comparisons).  
...
* Models and Methods
- Sketch ARGs models, branching process models and coevolutionary models that can be used to describe host and pathogen ARGs. Simulation with SLiM. Develop and extend coevolutionary model to be lineage-explicit (capable of describing genealogies forward or backwards in-time).
...
#+name: host-pathogen-hybrid-model
#+caption: Host pathogen model with both horizontal and vertical transmission and host and pathogen reproduction. This model aims to describe explicitly the reproduction of both hosts and pathogens and their genealogies using a two-step process of (1.) host reproduction and (2.) pathogen reproduction. The probability of vertical transmission is given by $c_{v}$ and a pathogen offspring (propagule) transmits to a random host horizontally with probability $c_{h}$.
#+attr_latex: :width 1\linewidth  :center t :placement [t]
